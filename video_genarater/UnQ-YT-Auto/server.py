...
def create_cors_middleware(allowed_origin: str):
    ...
    async def cors_middleware(request: web.Request, handler):
        ...
        # Allow requests from UnQCreator frontend (both development and production)
        if allowed_origin == "*":
...
class PromptServer():
    def __init__(self, loop):
        ...
        async def get_root(request):
            # Try to use our custom index.html first, fall back to default if not found
            custom_index_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "web_custom_override", "index.html")
            if os.path.exists(custom_index_path):
                response = web.FileResponse(custom_index_path)
            else:
