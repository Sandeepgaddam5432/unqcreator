          ...
          <CardContent className="space-y-4">
            {topVideos.map((video, index) => (
              <div key={index} className="flex items-start space-x-4">
                <div className="relative w-20 h-12 md:w-24 md:h-14 rounded-md overflow-hidden shrink-0">
                  <img 
                    src={video.thumbnail} 
                    alt={video.title}
                ...
                <div className="text-center">
                  <div className="text-sm font-semibold">{video.viralityScore}</div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className={`h-2.5 w-2.5 rounded-full ${
                      channel.status === 'healthy' ? 'bg-green-500' :
                      channel.status === 'warning' ? 'bg-yellow-500' :
                      'bg-red-500'
                    }`} />
                  </div>
                  {channel.issue && (
